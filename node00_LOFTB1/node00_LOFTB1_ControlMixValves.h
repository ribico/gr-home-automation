...
{
	float temp_floor_flow = temp_HVAC_Boiler_Floor_Flow;
	float temp_floor_return = temp_HVAC_Boiler_Floor_Return;
...
inline void AdjustCollectorToFloorFlowTemperature(float setpoint)
{
	...
	float error = setpoint - (temp_HVAC_Collector_Floor_Flow + temp_HVAC_Collector_Floor_Return/2);
