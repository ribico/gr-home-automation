...
//	nLowDigOut(HEATPUMP_CIRCULATION_PUMP_PIN, Souliss_T1n_OnCoil, HEATPUMP_CIRCULATION_PUMP);
	if(IsHpCirculationOn() || (IsHpCirculationAutoOn() && mInput(HEATPUMP_SANITARY_WATER) <= Souliss_T1n_AutoCmd) )
		dWrite(HEATPUMP_CIRCULATION_PUMP_PIN, LOW);
	else
	dWrite(HEATPUMP_CIRCULATION_PUMP_PIN, HIGH);
		
//	nLowDigOut(HEATPUMP_SANITARY_REQUEST_PIN, Souliss_T1n_OnCoil, HEATPUMP_SANITARY_WATER);
	if(IsSanitaryWaterOn() || (IsSanitaryWaterAutoOn() && (mInput(HEATPUMP_SANITARY_WATER) > Souliss_T1n_AutoCmd + SANITARY_PAUSE_CYCLES)) )
		dWrite(HEATPUMP_SANITARY_REQUEST_PIN, LOW);
	else
		dWrite(HEATPUMP_SANITARY_REQUEST_PIN, HIGH);
